...
class Websocket
{
    ...
    public function subscriptionTicker(array $markets, callable $callback)
    {
        foreach ($markets as $market) {
            $this->subscriptionTickerCallback[$market] = $callback;
        }
        $this->sendPublic(["action" => "subscribe", "channels" => [["name" => "ticker", "markets" => $markets]]]);
    }

    public function subscriptionTicker24h(array $markets, callable $callback)
    {
        foreach ($markets as $market) {
            $this->subscriptionTicker24hCallback[$market] = $callback;
        }
        $this->sendPublic(["action" => "subscribe", "channels" => [["name" => "ticker24h", "markets" => $markets]]]);
    }

    public function subscriptionAccount(array $markets, callable $callback)
    {
        foreach ($markets as $market) {
            $this->subscriptionAccountCallback[$market] = $callback;
        }
        $this->sendPrivate(["action" => "subscribe", "channels" => [["name" => "account", "markets" => $markets]]]);
    }

    public function subscriptionCandles(array $markets, $interval, callable $callback)
    {
        foreach ($markets as $market) {
            $this->subscriptionCandlesCallback[$market][$interval] = $callback;
        }
        $this->sendPublic(["action" => "subscribe", "channels" => [["name" => "candles", "interval" => [$interval], "markets" => $markets]]]);
    }

    public function subscriptionTrades(array $markets, callable $callback)
    {
        foreach ($markets as $market) {
            $this->subscriptionTradesCallback[$market] = $callback;
        }
        $this->sendPublic(["action" => "subscribe", "channels" => [["name" => "trades", "markets" => $markets]]]);
    }

    public function subscriptionBookUpdate(array $markets, callable $callback)
    {
        foreach ($markets as $market) {
            $this->subscriptionBookUpdateCallback[$market] = $callback;
        }
        $this->sendPublic(["action" => "subscribe", "channels" => [["name" => "book", "markets" => $markets]]]);
    }

    public function subscriptionBook(array $markets, callable $callback)
    {
        foreach ($markets as $market) {
